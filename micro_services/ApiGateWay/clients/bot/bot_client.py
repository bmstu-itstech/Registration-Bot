...
def push_answers(chat_id: int, bot_id: int, answers_list):
    channel = grpc.aio.insecure_channel(os.getenv('DATABASE_CONNECTION'))
    stub = pb2_grpc.BotWorkerStub(channel)
    response = stub.SetAnswers(pb2.SetAnswersRequest(bot_id=bot_id, tg_chat_id=chat_id, answers=answers_list))
...
def create_new_bot_asker(user_id: int, journal):
    channel = grpc.aio.insecure_channel(os.getenv('DATABASE_CONNECTION'))
    stub = pb2_grpc.BotWorkerStub(channel)
    response = stub.CreateBot(pb2.CreateBotRequest(from_user=user_id, journal=journal))
...
def get_bot(user_id: int, bot_id: int):
    channel = grpc.aio.insecure_channel(os.getenv('DATABASE_CONNECTION'))
    stub = pb2_grpc.BotGetterStub(channel)
    response = stub.GetBot(pb2.GetBotRequest(bot_id=bot_id, owner=user_id))
